...
impl<'a> Parser<'a> {
    ...
    pub fn parse(&mut self) -> Ast {
        ...
        while i < self.tokens.len() {
            let curr = self.tokens[i];
            match curr.typ {
                TokenType::Eof => break,
                TokenType::Word => match curr.word {
                    "version" => a.version = {
                        i += 1;
                        self.tokens[i].word.parse().unwrap()
                    ...
                    "header_name" => {
                        a.header_name = self.consume_braced_string(&mut i);
                    }
                    // "code_name" => {
                    //     self.next();
    ...
    fn consume_braced_string(&mut self, idx: &mut usize) -> String {
        let mut t = self.tokens[*idx];
        let start = t.start;
        let mut openbrace = 1;
        while t.typ != TokenType::Eof {
            *idx += 1;
